...
【サポート】
forum: https://discussions.unity.com/t/very-animation-animation-editor-released/680371
mail: support@alonesoft.sakura.ne.jp
X(twitter): @AlSoSupport


【更新履歴】
Version 1.3.8
- ADD : 初回起動やバージョン更新時にのみ表示されるクイックスタートへのリンク表示ガイドを追加
- FIX : 実行中の編集 : 追加レイヤーにClipが存在しない場合にエラーが発生する問題の修正

Version 1.3.7
- ADD : Unity6000.3対応
- FIX : AnimationWindow : Clamp, FootIK, AnimatorIK, OriginalIK : キーを編集した場合に値が変更されないTangentの更新では実行されないように変更 (MirrorとRoot Correctionは従来通り実行される)
- FIX : AnimationWindow : Mirror : FootIKとHandIKのカーブを編集した場合のミラー処理変更
- FIX : AnimationWindow : AnimatorIK, OriginalIK : 現在フレームに影響する前後のキーの編集で、現在のフレームにキーがない場合でもIKの更新を行うように修正
- FIX : Tools/Create New Clip/Mirror : FootIKとHandIKのカーブ作成処理変更
- FIX : AnimatorIK : TDoF有効時の動作を修正

Version 1.3.6
- FIX : SynchronizeAnimation : オブジェクトの状態によっては発生するエラーを修正

Version 1.3.5
- FIX : Unity 6000.3以降で発生するエラーの修正
- FIX : Curveが存在しない場合に新規作成されるデフォルトカーブの処理を修正
- FIX : 未使用処理の削除

Version 1.3.4
- ADD : Tools/Create New Clip : 作成されたClipにEditorGUIUtility.PingObjectを追加
- FIX : Tools/Create New Clip/Result : 処理の見直しによる最適化
- FIX : VA Tools : 最適化されているHierarchyのエラー処理修正
- FIX : SynchronizeAnimation, Preview : アニメーションリセット処理修正
- FIX : SynchronizeAnimation : 有効にしている状態かつ編集開始時点で動作しないことがある問題の修正
- FIX : SynchronizeAnimation : 最適化されているHierarchyは動作しないように変更
- FIX : Unity 6000.2以降で発生する警告の修正

Version 1.3.3
- FIX : Duplicate and Replace : Layers動作ではアクティブなレイヤーの参照のみが置き換えられるように修正
- FIX : コードのクリーンアップ

Version 1.3.2
- FIX : OnionSkinning : RenderPipeline取得ミスでOnionSkinningが表示されない可能性がある問題の修正
- FIX : Save Settings : 新規追加される状況でエラーが表示されることがある問題の修正
- FIX : Undo : エラーが表示される状態になることがある問題の修正
- FIX : Transform Rotation/RawQuaternions curve : 古いバージョンのUnityでのみ発生していたエラーへの対処を削除
- FIX : Transform Rotation/RawQuaternions curve : 更新でFilter by selectionが更新されていた問題の修正

Version 1.3.1
- FIX : 最低動作バージョンをUnity2021.3.0f1に変更
- FIX : Unity2023.1の初期バージョンでエラーが起きる問題の修正

Version 1.3.0
- ADD : Layers : AnimatorControllerのLayers設定を適用した状態での編集機能追加
- ADD : AnimatorIK/HandIK : 肩を動作させる機能と有効切り替えを追加
- ADD : AnimatorIK/Sync : Syncのデフォルト動作を設定する機能追加
- ADD : AnimatorIK/Animation Rigging : IKのUpdateで更新するConstraint情報を設定するWrite設定追加
- ADD : AnimatorIK/Animation Rigging : 作成時にAnimationRiggingの機能のShape表示を自動設定を追加
- ADD : AnimatorIK/Animation Rigging : 作成時にMirror設定を自動設定を追加
- ADD : Tools/Create New Clip : LayersやTimelineなどの結果を出力する用途のResultを追加
- ADD : Tools/Additive Reference Pose : 新規追加
- ADD : Tools/Cleanup : AvatarMaskで無効にされているカーブを削除する機能追加
- ADD : EditorWindow : Extraメニューを追加しCollisionはExtraへ移動、Collisionのショートカット削除
- ADD : ControlWindow/Hierarchy : カーブの自動作成や更新を制限するWrite Lock機能追加
- ADD : AnimatorIK and OriginalIK : IK選択中のショットカットKキーはUpdateを呼び出すように対応
- ADD : Tools/Root Motion/Motion Curves : MotionTQ選択中のショットカットKキーでのキー追加に対応
- ADD : Animation Rigging : 公式サポート、新機能追加
- ADD : Duplicate and Replace : Cleanupと回転の統一を行うかどうかのダイアログを追加
- ADD : BlendShape : iconの表示に変化したMeshのみに注視する2つのモードを追加、Addでの適用追加
- FIX : 最低動作バージョンをUnity2021.3に変更
- FIX : Root Correction : Clipを変更後の最初のフレームに正しくない結果となる事がある問題の修正
- FIX : Root Correction : AnimationWindowで現在フレーム以外を操作された場合の動作修正
- FIX : 編集時などにAnimatorWindowとParameterWindowをリセットしていた処理を削除
- FIX : AnimatorIK/Sync : 腕と足が真っすぐに伸びている状態でのSwivel取得の修正
- FIX : AnimatorIK/Space : Localタイプで必要ない更新が行われないように修正
- FIX : Tools/Humanoid IK/HandIK : HandQの値が正常ではなかった問題の修正
- FIX : AnimatorIK/Animation Rigging : IKのUpdateで設定されるHintの位置情報を修正
- FIX : Timeline : TimelineとAnimationClipでフレームレートが違う場合の動作に対応
- FIX : Pose/Reset : BindPoseとTPoseはPrefabからではなくSceneObjectから取得するように変更
- FIX : Animation Rigging : 正常終了以外の状況でのメモリリーク修正
- FIX : Rendererのenabledをアニメーションで操作された場合にスケルトンの情報が重なって表示されることがある問題の修正
- FIX : AnimationCurveが追加される処理をAnimationWindowの処理と合わせる修正
- FIX : Keyframeが追加される処理をAnimationWindowの処理と合わせる修正
- FIX : Tools/Keyframe Reduction/Other Curves : 値１が連続するのみのカーブが削減されないことがある問題の修正
- FIX : AnimatorControllerのLayerのSyncが有効な場合の各種動作の修正
- FIX : Unloadされたシーンが存在する場合のエラー修正
- FIX : HandPose and BlendShape : Iconが正しく表示されないことがある問題の修正

Version 1.2.28
- ADD : Tutorial : YouTubeのプレイリストを開くリンク選択時に了承確認ダイアログを追加
- FIX : AnimatorIK : 右手の子供の武器を左手IKの親にするような両手持ち状況でのSpace parentの不具合修正
- FIX : OriginalIK : 右手の子供の武器を左手IKの親にするような両手持ち状況でのSpace parentの不具合修正

Version 1.2.27
- FIX : 選択処理でMeshTopologyがTrianglesではない場合に発生するエラーを修正

Version 1.2.26
- FIX : Onion Skinning : URPとHDRPの問題の修正、表示の修正

Version 1.2.25
- ADD : Unity6 対応
- ADD : Tutorial Video Playlistのリンクを追加
- FIX : AnimationWindow : Property Style : Filter : IK有効時に切り替えを繰り返しちらつく状態になる問題を修正
- FIX : AnimationWindow : Property Style : Filter : カーブがなく編集するとカーブが作成される状況での複数選択で切り替えを繰り返しちらつく状態になる問題を修正
- FIX : AnimationWindow : Property Style : Filter : ボーン選択中に選択外のカーブが変更された場合に切り替えを繰り返しちらつく状態になる問題を修正
...
Version 1.2.22
- FIX : Unity2022.3以降での不要なLogの出力を無効化

Version 1.2.21
- ADD : Unity2023.1対応
- ADD : Timeline : 編集時にTimelineWindowを自動的にLockする処理の追加
- FIX : Timeline : Unity2023.1以降でショートカットキーで再生が行えない問題の修正
