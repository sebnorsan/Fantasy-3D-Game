...
[Update History]
Version 1.3.8
- ADD : Added a guide to inform users of the Quick Start link that is only displayed when first starting the app or updating the version.
- FIX : Editing while playing : Fixed an issue where an error would occur if there was no Clip in the added layer.

Version 1.3.7
- ADD : Unity6000.3 support
- FIX : AnimationWindow : Clamp, FootIK, AnimatorIK, OriginalIK : Changed so that it is not executed when updating Tangent, where the value does not change when editing a key (Mirror and Root Correction are still executed as before).
- FIX : AnimationWindow : Mirror : Changes to mirror processing when editing FootIK and HandIK curves
- FIX : AnimationWindow : AnimatorIK, OriginalIK : When editing previous and next keys that affect the current frame, IK is now updated even if there is no key on the current frame.
- FIX : Tools/Create New Clip/Mirror : Changes to FootIK and HandIK curve creation processing
- FIX : AnimatorIK : Fixed behavior when TDoF is enabled

Version 1.3.6
- FIX : SynchronizeAnimation : Fixed an error that occurred depending on the object state.

Version 1.3.5
- FIX : Fixed an error that occurs in Unity 6000.3 and later.
- FIX : Fixed the handling of default curves created when a curve does not exist
- FIX : Delete unused processes

Version 1.3.4
- ADD : Tools/Create New Clip : Add EditorGUIUtility.PingObject to the created Clip.
- FIX : Tools/Create New Clip/Result : Optimization through review of processing
- FIX : VA Tools : Optimized Hierarchy error handling fixes
- FIX : SynchronizeAnimation, Preview : Animation reset process fix
- FIX : SynchronizeAnimation : Fixed an issue where the function would not work when editing was started even if it was enabled.
- FIX : SynchronizeAnimation : Changed the optimized Hierarchy to not work.
- FIX : Fixed warnings that occur in Unity 6000.2 and later
...
Version 1.2.28
- ADD : Tutorial : Added confirmation dialog when selecting a link to open a YouTube playlist
- FIX : AnimatorIK : Fixed a bug with Space Parent when using a right-handed child weapon as the left-handed IK parent.
- FIX : OriginalIK : Fixed a bug with Space Parent when using a right-handed child weapon as the left-handed IK parent.

Version 1.2.27
- FIX : Fixed an error that occurred when MeshTopology was not Triangles during selection processing.

Version 1.2.26
- FIX : Onion Skinning : Fixed URP and HDRP issues, display fixes

Version 1.2.25
- ADD : Unity6 support
- ADD : Added links to Tutorial Video Playlist
- FIX : AnimationWindow : Property Style : Filter : Fixed an issue where IK would repeatedly switch and flicker when enabled.
- FIX : AnimationWindow : Property Style : Filter : Fixed an issue where multiple selections would flicker repeatedly when there was no curve and editing would create a curve
- FIX : AnimationWindow : Property Style : Filter : Fixed an issue where bones would repeatedly flicker when a curve outside of the selection was changed while the bone was selected.
