...
namespace VeryAnimation
{
    ...
    {
        public class ArrowMesh : IDisposable
        {
        ...
        public static Func<object, T> CreateGetFieldDelegate<T>(FieldInfo fi)
        {
            ...
            var dynamicMethod = new DynamicMethod(methodName, typeof(T), new Type[] { typeof(object) }, true);
        ...
        public static Action<object, T> CreateSetFieldDelegate<T>(FieldInfo fi)
        {
            ...
            var dynamicMethod = new DynamicMethod(methodName, null, new Type[] { typeof(object), typeof(T) }, true);
            ILGenerator gen = dynamicMethod.GetILGenerator();
            if (fi.IsStatic)
            {
        ...
        public static AnimationClip CreateNewClipAtPath(string clipPath)
        {
            var newClip = new AnimationClip();
            {
                ...
                info.loopTime = true;
                UAnimationUtility uAnimationUtility = new();
        ...
        public static int GetTimeFrame(float time, float frameRate)
        {
            return Mathf.FloorToInt(time / (1f / frameRate));
        }
        public static float GetFrameTime(int frame, float frameRate)
        {
            return SnapToFrame(frame * (1f / frameRate), frameRate);
        }
