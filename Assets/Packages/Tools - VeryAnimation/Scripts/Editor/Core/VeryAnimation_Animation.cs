...
namespace VeryAnimation
{
    internal partial class VeryAnimation
    {
        ...
        public AnimationClip[] GetLayerAnimationClips(int layerIndex)
        {
                ...
                #region Set
                {
                    ...
                    foreach (var pair in humanoidFootIK.updateFrame.Frames)
                    {
                        ...
                        for (var ikIndex = AnimatorIKIndex.LeftFoot; ikIndex <= AnimatorIKIndex.RightFoot; ikIndex++)
                        {
                            ...
                            for (int i = 0; i < 3; i++)
                            {
                                SetKeyframe(humanoidFootIK.ikCurves[index].ikT[i], time, ikT[i]);
                            }
                            for (int i = 0; i < 4; i++)
                            {
                                SetKeyframe(humanoidFootIK.ikCurves[index].ikQ[i], time, ikQ[i]);
                            }
                        }
                    }
                    Skeleton.SetTransformStart();
        ...
        public bool GetPlayingAnimationInfo(out PlayingAnimationInfo[] playingAnimationsInfo)
        {
            ...
            if (VAW.Animator != null && VAW.Animator.runtimeAnimatorController != null && VAW.Animator.isInitialized)
            {
                ...
                if (ac != null && VAW.Animator.layerCount > 0)
                {
                    ...
                    for (int layerIndex = 0; layerIndex < layers.Length; layerIndex++)
                    {
                        ...
                        bool resultFindStateMachine = false;
                        if (layers[layerIndex].syncedLayerIndex >= 0)
                            resultFindStateMachine = FindStateMachine(layers[layers[layerIndex].syncedLayerIndex].stateMachine);
                        else
                            resultFindStateMachine = FindStateMachine(layers[layerIndex].stateMachine);
                        if (resultFindStateMachine)
                        {
            ...
            else if (VAW.Animation != null)
            {
                ...
                foreach (AnimationState state in VAW.Animation)
                {
                    ...
                    playingAnimationsInfo = new PlayingAnimationInfo[]
                    {
                        new()
                        {
                            clip = dstClip,
        ...
        public int GetTimeFrame(float time)
        {
            return UAw.TimeToFrameRound(time);
        }
        ...
        public int FindKeyframeIndexValueOnly(AnimationCurve curve, AnimationCurve findCurve, int findIndex)
        {
            var index = FindKeyframeAtTime(curve, findCurve[findIndex].time);
            if (index >= 0)
            {
        ...
        private int AddInbetweenKeyframe(AnimationCurve curve, float time)
        {
        ...
        public int SetKeyframe(AnimationCurve curve, float time, float value)
        {
            var keyIndex = FindKeyframeAtTime(curve, time);
            if (keyIndex < 0)
            {
        ...
        public int SetKeyframe(AnimationCurve curve, Keyframe keyframe)
        {
        ...
        public void RemoveBetweenKeyframe(AnimationCurve curve, float min, float max)
        {
        ...
        public void SetKeyframeTangentModeLinear(AnimationCurve curve, float time)
        {
            SetKeyframeTangentModeLinear(curve, FindKeyframeAtTime(curve, time));
        }
        ...
        public void SetKeyframeTangentModeClampedAuto(AnimationCurve curve, float time)
        {
            SetKeyframeTangentModeClampedAuto(curve, FindKeyframeAtTime(curve, time));
        }
        public void SetKeyframeTangentFlat(AnimationCurve curve, int keyIndex)
        {
            if (keyIndex < 0 || keyIndex >= curve.length) return;
            AnimationUtility.SetKeyBroken(curve, keyIndex, false);
        ...
        public void SetKeyframeTangentFlat(AnimationCurve curve, float time)
        {
            SetKeyframeTangentFlat(curve, FindKeyframeAtTime(curve, time));
        }
        ...
        private void ActionAllAnimatorState(AnimationClip clip, Action<UnityEditor.Animations.AnimatorState> action)
        {
            ...
            for (int layerIndex = 0; layerIndex < layers.Length; layerIndex++)
            {
                ...
                if (layers[layerIndex].syncedLayerIndex >= 0)
                    ActionStateMachine(layers[layers[layerIndex].syncedLayerIndex].stateMachine);
                else
                    ActionStateMachine(layers[layerIndex].stateMachine);
            }
        }
            ...
            if (curve != null)
            {
                if (data.beforeCurve != null)
                {
                    if (valueCheckOnly)
                    {
                        ...
                        {
                            if (FindKeyframeIndexValueOnly(data.beforeCurve, curve, i) < 0)
                            {
        ...
        private void CreateEditorCurveBindingPropertyNames()
        {
            ...
            }
            {
                ...
                {
                    var dofCount = (URotationCurveInterpolation.Mode)i switch
                    {
                        URotationCurveInterpolation.Mode.RawQuaternions => 4,
                        _ => 3,
                    };
                    ...
                    for (int dofIndex = 0; dofIndex < dofCount; dofIndex++)
                    {
                        ...
                        EditorCurveBindingTransformRotationPropertyNames[i][dofIndex] = URotationCurveInterpolation.PrefixForInterpolation[i] + DofIndex2String[dofIndex][1..];
                    }
                }
            }
        }
        ...
        public int GetMotionTDofIndexFromCurveBinding(EditorCurveBinding binding)
        {
            ...
            for (int dofIndex = 0; dofIndex < 3; dofIndex++)
            {
                if (binding == AnimationCurveBindingAnimatorMotionT[dofIndex])
                    return dofIndex;
            }
            return -1;
        }
        public int GetMotionQDofIndexFromCurveBinding(EditorCurveBinding binding)
        {
            ...
            for (int dofIndex = 0; dofIndex < 4; dofIndex++)
            {
                if (binding == AnimationCurveBindingAnimatorMotionQ[dofIndex])
                    return dofIndex;
            }
            return -1;
        }
        ...
        {
            binding.propertyName = binding.propertyName[..^DofIndex2String[dofIndex].Length];
        ...
        public EditorCurveBinding? GetMirrorAnimationCurveBinding(EditorCurveBinding binding)
        {
            if (binding.type == typeof(Animator))
            {
                ...
                else if ((ikQIndex = GetIkQIndexFromCurveBinding(binding)) != AnimatorIKIndex.None)
                {
                    ...
                    return AnimationCurveBindingAnimatorIkQ(mikQIndex, mdof);
                }
            ...
            else if (IsSkinnedMeshRendererBlendShapeCurveBinding(binding))
            {
                ...
                if (!nameTable.TryGetValue(PropertyName2BlendShapeName(binding.propertyName), out string mirrorName)) return null;
        ...
        public void UpdateSyncEditorCurveClip()
        {
            ...
            if (editorCurveWasModifiedDic != null && editorCurveWasModifiedDic.Count > 0)
            {
                ...
                if (bindings.Count > 0)
                    SetAnimationWindowSynchroSelection(bindings.ToArray());
            }
        }
        #endregion
            ...
            if ((flags & PoseFlags.BlendShape) != 0)
            {
                if (poseTemplate.blendShapePaths != null && poseTemplate.blendShapeValues != null)
                {
                    foreach (var renderer in VAW.GameObject.GetComponentsInChildren<SkinnedMeshRenderer>(true))
                    {
                        ...
                        {
                            if (blendShapeAdd)
                                SetAnimationValueBlendShapeIfNotOriginal(renderer, poseTemplate.blendShapeValues[index].names[i], GetAnimationValueBlendShape(renderer, poseTemplate.blendShapeValues[index].names[i]) + poseTemplate.blendShapeValues[index].weights[i]);
                            else
        ...
        public void SetSynchroIKtargetOriginalIK(int ikTarget)
        {
            ...
            if (IsHuman)
            {
                ...
                {
                    if (originalIK.ikData[ikTarget].joints[i] == null) continue;
                    SetSynchroIKtargetBone(BonesIndexOf(originalIK.ikData[ikTarget].joints[i].bone));
                }
            }
        ...
        public void CreateDefaultCurves(AnimationClip clip, EditorCurveBinding[] bindings)
        {
            if (clip == CurrentClip)
            {
                ClearEditorCurveCache();
        ...
        private void LoadTmpCurvesFullDof(EditorCurveBinding binding, int count, EditorCurveBinding? subBinding = null)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                tmpCurves.bindings[i].propertyName = tmpCurves.bindings[i].propertyName[..^DofIndex2String[i].Length] + DofIndex2String[i];
                ...
                if (subBinding.HasValue)
                {
                    ...
                    tmpCurves.subBindings[i].propertyName = tmpCurves.subBindings[i].propertyName[..^DofIndex2String[i].Length] + DofIndex2String[i];
        ...
        public AnimationCurve GetAnimationCurveAnimatorRootT(int dof, bool notNull = true)
        {
            ...
            if (curve == null && notNull)
            {
                ...
                SetKeyframe(curve, 0f, defaultValue[dof]);
                SetKeyframe(curve, Mathf.Max(CurrentTime, GetTotalClipLength()), defaultValue[dof]);
                SetEditorCurveCache(binding, curve);
                //Created
                UAw.ForceRefresh();
        ...
        public AnimationCurve GetAnimationCurveAnimatorMuscle(int muscleIndex, bool notNull = true)
        {
            ...
            {
                curve = new();
                SetKeyframe(curve, 0f, 0f);
                SetKeyframe(curve, Mathf.Max(CurrentTime, GetTotalClipLength()), 0f);
                SetEditorCurveCache(binding, curve);
                //Created
                UAw.ForceRefresh();
                SetAnimationWindowSynchroSelection();
        ...
        public AnimationCurve GetAnimationCurveBlendShape(SkinnedMeshRenderer renderer, string name, bool notNull = true)
        {
            ...
            if (curve == null && notNull)
            {
                ...
                SetKeyframe(curve, 0f, defaultValue);
                SetKeyframe(curve, Mathf.Max(CurrentTime, GetTotalClipLength()), defaultValue);
                SetEditorCurveCache(binding, curve);
                //Created
                UAw.ForceRefresh();
                SetAnimationWindowSynchroSelection();
        ...
        public AnimationCurve GetAnimationCurveCustomProperty(EditorCurveBinding binding, bool notNull = true)
        {
            ...
            if (curve == null && notNull)
            {
                curve = new();
                AnimationUtility.GetFloatValue(VAW.GameObject, binding, out float value);
                SetKeyframe(curve, 0f, value);
                SetKeyframe(curve, Mathf.Max(CurrentTime, GetTotalClipLength()), value);
                SetEditorCurveCache(binding, curve);
                //Created
                UAw.ForceRefresh();
                SetAnimationWindowSynchroSelection();
