...
{
    [Serializable]
    internal class VeryAnimationToolWindow : EditorWindow
    {
        ...
        private void ToolsResetPose(GameObject activeGameObject, GameObject activePrefab)
        {
            ...
            {
                Dictionary<string, Transform> srcList = new();
                Dictionary<string, Transform> dstList = new();
                {
                    static void SaveTransform(Dictionary<string, Transform> transforms, Transform t, Transform root)
                    {
                ...
                {
                    if (srcList.TryGetValue(pair.Key, out Transform srcT))
                    {
                        var dstT = pair.Value;
...
#endif
            void ReplaceAnimation(Animation animation)
            {
                            ...
                            foreach (var state in stateMachine.states)
                            {
                                ...
                                else
                                {
                                    if (motion == toolReplaceReference_OldClip)
                                    {
                                        if (layerIndex > 0 && layers[layerIndex].syncedLayerIndex >= 0)
                                        {
                                            Undo.RecordObject(controller, "Duplicate and Replace");
                                            layers[layerIndex].SetOverrideMotion(state.state, toolReplaceReference_NewClip);
                                            updateLayers = true;
                                        }
                                        else
                                        {
